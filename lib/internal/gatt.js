...
function GattServerDb(registerOnConnected1Fn, registerOnConnected2Fn, registerOnDisconnectedFn, registerOnBondedFn, registerAttDbFn) {
	...
	var minConIntervalValue = Buffer.from([0, 0]);
	var maxConIntervalValue = Buffer.from([0, 0]);
	var slaveLatencyValue = Buffer.from([0, 0]);
	var timeoutMultiplierValue = Buffer.from([0, 0]);
	minConIntervalValue.writeUint16LE(6);
	maxConIntervalValue.writeUint16LE(3200);
	slaveLatencyValue.writeUint16LE(0);
	timeoutMultiplierValue.writeUint16LE(10);
	...
	this.setPreferredConnectionParameters = function(minConInterval, maxConInterval, slaveLatency, timeoutMultiplier) {
		validate(Number.isInteger(minConInterval) && minConInterval >= 6 && minConInterval <= 3200, 'Minimum connection interval must be a 16-bit integer between 6 and 3200');
		validate(Number.isInteger(maxConInterval) && maxConInterval >= minConInterval && maxConInterval >= 6 && maxConInterval <= 3200, 'Maximum connection interval must be a 16-bit integer between 6 and 3200');
		validate(Number.isInteger(slaveLatency) && slaveLatency >= 0 && slaveLatency <= 1000, 'Slave latency must be a 16-bit integer between 0 and 1000');
		validate(Number.isInteger(timeoutMultiplier) && timeoutMultiplier >= 10 && timeoutMultiplier <= 3200, 'Timeout multiplier must be a 16-bit integer between 0 and 1000');
		minConIntervalValue = minConInterval;
		maxConIntervalValue = maxConInterval;
		slaveLatencyValue = slaveLatency;
		timeoutMultiplierValue = timeoutMultiplier;
	}
			...
			characteristics: [
				...
				},
				{
					...
					onRead: function(connection, callback) {
						...
						slaveLatencyValue.writeUint16LE(0);
						timeoutMultiplierValue.writeUint16LE(10);
						callback(0, Buffer.concat([ minConIntervalValue, maxConIntervalValue, slaveLatencyValue, timeoutMultiplierValue ]), 8);
					},
					maxLength: 8,
					descriptors: []
				}
